...
export default 
{
    ...
    handleDelete(index, row) {
      ...
      var api_url = this.host.concat("/api/attr_data_delete?id=").concat(row.id).concat("&attr_type=").concat(row.attr_type);
    ...
    handleSave(index, row) {
      ...
      var api_url = this.host.concat("/api/attr_data_update?id=").concat(row.id)
                    .concat("&code=").concat(row.code).concat("&code_type=").concat(row.code_type)
                    .concat("&attr_type=").concat(row.attr_type).concat("&title=").concat(row.title)
                    .concat("&date=").concat(row.date).concat("&url=").concat(row.url)
    ...
    handleOk(index, row) {
      ...
      var api_url = this.host.concat("/api/attr_data_confirm?id=").concat(row.id).concat("&attr_type=").concat(row.attr_type);
      this.axios.get(api_url)
      .then(response => {
        if((response.status != 200) || (response.data.result != 0)){
          this.$message({
    ...
    initEditAttribute() {
      ...
      var api_url = host.concat("/api/attr_data_for_check");
      this.axios.get(api_url)
      .then(response => {
        if((response.status == 200) && (response.data.result == 0)){
